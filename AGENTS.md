...
Use `python ops/worker_upload_load.py --workers <n> --uploads <m> --concurrency <c> --operation heartbeat|event|artifact|mixed|lease --artifact-kib <k> --event-kib <k>` from `pullwise-server` to measure v1 worker control-plane and artifact upload throughput against a real local `ThreadingHTTPServer` and temporary SQLite DB. New v1 artifact uploads store content bytes outside SQLite under `PULLWISE_REVIEW_ARTIFACT_STORAGE_DIR` or next to `PULLWISE_DB_PATH`; `review_artifacts.payload_json` must not contain `content_base64`, and `content_path` is server-internal. Authenticated gzip worker artifact/result uploads default to a 50 MiB decompressed body limit, but worker hot paths are still serialized by the SQLite lock. The July 2026 300-worker local probes remain a failing scale signal: artifact 32 KiB uploads completed 300/300 with p50 roughly 72-91s and p95 roughly 94-112s, 300-worker progress events did not complete within an 8-minute local timeout after scan-mirror throttling, and lease storms still hit request timeouts even after eliminating offline 503s. Lease requests may refresh only the requesting worker presence; do not run full recovery sweeps inline on the claim hot path. Treat this probe as the regression/operational check before increasing worker fleet size, artifact size, heartbeat frequency, progress-event frequency, or lease claim rate.
