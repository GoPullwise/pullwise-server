...
class AgentFirstGateDecisionFacadesTest(unittest.TestCase):
    ...
    def test_terminal_gate_binds_ledger_cas_task_active_state_and_derived_state(
        self,
    ) -> None:
        base_snapshot, base_context = self.terminal_inputs()
        ...
        cas["effect_ledger"] = self.effect_ledger(cas_snapshot["task_id"], [])
        ...
        results = self.assert_operation_parity(
            [
                {
                    "kind": "terminal",
