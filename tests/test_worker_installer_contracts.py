...
class WorkerInstallerContractsTest(unittest.TestCase):
    ...
    def test_installer_scopes_codex_sqlite_state_and_config(self) -> None:
        ...
        self.assertNotIn('codex_cli_override_enabled() {', script)
        self.assertIn('if provider_chain_has codex; then', script)
    ...
    def test_installer_bootstraps_python_and_uses_latest_scoped_codex_cli_by_default(self) -> None:
        script = app.worker_install_script()
        ...
        self.assertIn('run_as_service_user "$CODEX_COMMAND" --version', script)
        self.assertIn('write_optional_env_value PULLWISE_CODEX_COMMAND "$CODEX_COMMAND"', script)
        self.assertIn('write_optional_env_value PULLWISE_CODEX_RELEASE "$CODEX_RELEASE"', script)
        self.assertLess(
            script.index('\n  ensure_codex_cli\n'),
            script.index('run_as_service_user "$PYTHON_BIN" -m pip install --upgrade --force-reinstall --no-cache-dir "$WORKER_PACKAGE"'),
        )
