...
class WorkerAdminRoutesTest(unittest.TestCase):
    ...
    def test_admin_can_create_worker_and_token_is_only_returned_once_as_hash(self) -> None:
        ...
        self.assertEqual(payload["suggested_env"]["PULLWISE_PROVIDER_CHAIN"], "codex,opencode")
    ...
    def test_public_install_script_contains_deploy_assets_but_no_worker_secrets(self) -> None:
        ...
        self.assertIn('install -d -m 0755 -o root -g root "$BASE_CONFIG_DIR" "$BASE_DATA_DIR" "$BASE_LOG_DIR"', install.text_payload)
        ...
        self.assertIn("path_command", install.text_payload)
        ...
        self.assertIn('path_command codex', install.text_payload)
        self.assertIn('path_command opencode', install.text_payload)
        ...
        self.assertNotIn("OpenCode current model provider:", install.text_payload)
