...
class WorkerAdminRoutesTest(unittest.TestCase):
    ...
    def test_admin_can_create_worker_and_token_is_only_returned_once_as_hash(self) -> None:
        ...
        self.assertEqual(payload["local_server_url"], "http://127.0.0.1:8080")
        self.assertEqual(payload["local_install_url"], "http://127.0.0.1:8080/install-worker.sh")
        self.assertIn("http://127.0.0.1:8080/install-worker.sh", payload["local_install_command"])
        self.assertIn("--server 'http://127.0.0.1:8080'", payload["local_install_command"])
        ...
        self.assertEqual(payload["suggested_env"]["PULLWISE_LOCAL_SERVER_URL"], "http://127.0.0.1:8080")
