...
class WorkerAdminRoutesTest(unittest.TestCase):
    ...
    def test_public_install_script_contains_deploy_assets_but_no_worker_secrets(self) -> None:
        ...
        self.assertIn('SERVICE_USER="pullwise-worker-$SAFE_WORKER_ID"', install.text_payload)
        self.assertIn('CONFIG_DIR="/etc/pullwise-worker/$SAFE_WORKER_ID"', install.text_payload)
        self.assertIn('DATA_DIR="/var/lib/pullwise-worker/$SAFE_WORKER_ID"', install.text_payload)
        self.assertIn('LOG_DIR="/var/log/pullwise-worker/$SAFE_WORKER_ID"', install.text_payload)
        self.assertIn('SERVICE_FILE="/etc/systemd/system/pullwise-worker-$SAFE_WORKER_ID.service"', install.text_payload)
        self.assertIn('BIN_PATH="/usr/local/bin/pullwise-worker-$SAFE_WORKER_ID"', install.text_payload)
        ...
        self.assertIn("pullwise_worker-0.4.2-py3-none-any.whl", install.text_payload)
        ...
        self.assertNotIn("command -v codex", install.text_payload)
        self.assertNotIn("command -v opencode", install.text_payload)
        self.assertIn("codex login --device-auth", install.text_payload)
        ...
        self.assertIn('load_worker_env "${PULLWISE_WORKER_ENV_FILE:-/etc/pullwise-worker/', install.text_payload)
