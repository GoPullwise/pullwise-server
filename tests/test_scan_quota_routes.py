...
class ScanQuotaRoutesTest(unittest.TestCase):
    ...
    def test_private_worker_management_routes_are_not_available(self) -> None:
        ...
        for method, path, body in (
            ...
            ("POST", "/private-workers/wk_1/disable", {}),
            ("DELETE", "/private-workers/wk_1", {}),
        ):
            handler = RouteHarness(body, cookie=cookie, path=path)
            app.PullwiseHandler.route(handler, method)
            self.assertEqual(handler.status, HTTPStatus.NOT_FOUND)

        with self.assertRaisesRegex(ValueError, "Private workers are not supported"):
            db.create_worker(
                {
