...
export async function verifyCompletionProposalContext(
  ...
  verificationRequire(
    checked.task_id === snapshot.task_id &&
      checked.task_id === currentAttempt.task_id &&
      checked.task_id === ownerDoc.task_id &&
      checked.task_id === request.task_id,
    VERIFICATION_CONTEXT_INVALID,
    "$.task_id",
  );
  verificationRequire(
    currentAttempt.attempt_id === ownerDoc.attempt_id &&
      ownerDoc.attempt_id === checked.attempt_id,
    VERIFICATION_CONTEXT_INVALID,
    "$.attempt_id",
  );
  ...
  verificationRequire(
    checked.native_epoch === currentAttempt.native_epoch &&
      currentAttempt.native_epoch === ownerDoc.native_epoch,
    VERIFICATION_CONTEXT_INVALID,
    "$.native_epoch",
  );
  ...
  verificationRequire(
    pre.task_id === checked.task_id &&
      pre.proposal_id === checked.proposal_id &&
      pre.attempt_id === checked.attempt_id &&
      pre.native_epoch === checked.native_epoch,
    VERIFICATION_CONTEXT_INVALID,
    "$.pre_observation_manifest",
  );
  verificationRequireIdList(
    verificationAssessmentIds(checked.requirement_claims),
  ...
  checked.requirement_claims.forEach((item, index) => {
    verificationRequire(
      item.evidence_ids.every((id) => preIds.has(id)),
      VERIFICATION_CONTEXT_INVALID,
  ...
  for (const [path, timestamp] of [
    ...
    ["$.attempt.lease_acquired_at", currentAttempt.lease_acquired_at],
    ["$.attempt.started_at", currentAttempt.started_at],
    ["$.attempt.ended_at", currentAttempt.ended_at],
    ["$.owner.started_at", ownerDoc.started_at],
    ["$.owner.ended_at", ownerDoc.ended_at],
...
export async function verifyVerifierInputContext(
  ...
  verificationRequire(
    checked.task_id === proposalDoc.task_id &&
      checked.task_id === request.task_id &&
      checked.task_id === ledger.task_id &&
      checked.task_id === charter.task_id &&
      checked.task_id === plan.task_id,
    VERIFICATION_CONTEXT_INVALID,
    "$.task_id",
  );
  verificationRequire(
    checked.proposal_id === proposalDoc.proposal_id &&
      checked.proposal_id === plan.proposal_id,
    VERIFICATION_CONTEXT_INVALID,
    "$.proposal_id",
  );
  ...
  if (change === null) {
    verificationRequire(
      checked.change_set.availability !== "available" && proposalDoc.change_set_ref === null,
      VERIFICATION_CONTEXT_INVALID,
      "$.change_set",
    );
  } else {
  ...
  verificationRequire(
    plan.policy_digest === proposalDoc.policy_digest && proposalDoc.policy_digest === policy.digest,
    VERIFICATION_CONTEXT_DIGEST_INVALID,
    "$.quality_policy_plan_digest",
  );
  ...
  verificationRequire(
    pre.task_id === proposalDoc.task_id &&
      pre.proposal_id === proposalDoc.proposal_id &&
      pre.attempt_id === proposalDoc.attempt_id &&
      pre.native_epoch === proposalDoc.native_epoch,
    VERIFICATION_CONTEXT_INVALID,
...
export async function verifyVerifierWorkContext(
  report, verifierInput, proposal, finalObservationManifest,
) {
  ...
  verificationRequire(
    checked.task_id === inputDoc.task_id &&
      checked.task_id === proposalDoc.task_id &&
      checked.task_id === finalManifest.task_id,
    VERIFICATION_CONTEXT_INVALID,
    "$.task_id",
  );
  verificationRequire(
    checked.proposal_id === inputDoc.proposal_id &&
      checked.proposal_id === proposalDoc.proposal_id &&
      checked.proposal_id === finalManifest.proposal_id,
    VERIFICATION_CONTEXT_INVALID,
    "$.proposal_id",
  );
  verificationRequire(checked.slot_id === inputDoc.slot_id, VERIFICATION_CONTEXT_INVALID, "$.slot_id");
  verificationRequire(
    finalManifest.attempt_id === proposalDoc.attempt_id &&
      finalManifest.native_epoch === proposalDoc.native_epoch,
    VERIFICATION_CONTEXT_INVALID,
    "$.final_observation_manifest",
  );
  verificationRequireIdList(
    verificationAssessmentIds(checked.provisional_requirement_assessments),
    inputDoc.requirement_ids,
  ...
  verificationRequire(
    [...ownIds].every((id) => finalIds.has(id)),
    VERIFICATION_CONTEXT_INVALID,
    "$.own_observation_ids",
  );
  checked.own_observation_ids.forEach((id) => {
    ...
    verificationRequire(
      actor.kind === "quality_verifier" && actor.session_id === checked.verifier_session_id,
      VERIFICATION_CONTEXT_INVALID,
      "$.own_observation_ids",
    );
  });
  checked.provisional_requirement_assessments.forEach((item, index) => {
    ...
    verificationRequire(
      [...evidence].every((id) => finalIds.has(id) && ownIds.has(id)),
      VERIFICATION_CONTEXT_INVALID,
...
export async function verifyAttestationContext(
  ...
  verificationRequire(
    checked.task_id === inputDoc.task_id &&
      checked.task_id === work.task_id &&
      checked.task_id === proposalDoc.task_id &&
      checked.task_id === plan.task_id &&
      checked.task_id === manifest.task_id,
    VERIFICATION_CONTEXT_INVALID,
    "$.task_id",
  );
  verificationRequire(
    checked.proposal_id === inputDoc.proposal_id &&
      checked.proposal_id === work.proposal_id &&
      checked.proposal_id === proposalDoc.proposal_id &&
      checked.proposal_id === plan.proposal_id &&
      checked.proposal_id === manifest.proposal_id,
    VERIFICATION_CONTEXT_INVALID,
    "$.proposal_id",
  );
  ...
  verificationRequire(
    checked.source_state_id === final.source_state_id &&
      final.source_state_id === proposalDoc.final_source_state_id,
    VERIFICATION_CONTEXT_INVALID,
    "$.source_state_id",
  );
  verificationRequireIdList(checked.execution_state_ids, states.map((item) => item.execution_state_id), "$.execution_state_ids");
  ...
  checked.own_observation_ids.forEach((id) => {
    ...
    verificationRequire(
      actor !== undefined &&
        actor.kind === "quality_verifier" &&
        actor.session_id === checked.verifier_session_id,
      VERIFICATION_CONTEXT_INVALID,
      "$.own_observation_ids",
    );
  });
  verificationRequireIdList(
    verificationAssessmentIds(checked.requirement_verdicts),
    inputDoc.requirement_ids,
  ...
  checked.requirement_verdicts.forEach((item, index) => {
    ...
    verificationRequire(
      [...evidence].every((id) => finalIds.has(id) && ownIds.has(id)),
      VERIFICATION_CONTEXT_INVALID,
      `$.requirement_verdicts[${index}].evidence_ids`,
    );
  });
...
export async function verifyAttestationManifestContext(
  ...
  const planSlots = plan.slots.map((item) => item.slot_id);
  const planSlotSet = new Set(planSlots);
  const manifestSlots = [];
  checked.attestations.forEach((entry, index) => {
    ...
    verificationRequire(
      entry.slot_id === document.slot_id && entry.run_status === document.run_status,
      VERIFICATION_CONTEXT_INVALID,
      `$.attestations[${index}]`,
    );
  ...
  verificationRequire(
    manifestSlots.every((slotId) => planSlotSet.has(slotId)) &&
      new Set(manifestSlots).size === manifestSlots.length,
    VERIFICATION_CONTEXT_INVALID,
    "$.attestations",
  );
  ...
  const expectedRequirements = [...new Set(
    plan.slots.flatMap((slot) => slot.requirement_ids),
  )].sort(verificationTextCompare);
  verificationRequireIdList(
    checked.requirement_aggregates.map((item) => item.requirement_id),
  ...
  checked.requirement_aggregates.forEach((aggregate, index) => {
    const requiredSlotIds = plan.slots
      .filter((slot) => slot.requirement_ids.includes(aggregate.requirement_id))
    ...
    requiredSlotIds.forEach((slotId) => {
      ...
      const verdict = document.requirement_verdicts.find(
        (item) => item.requirement_id === aggregate.requirement_id,
      );
    ...
    const expectedVerdict = missing
      ? "UNVERIFIABLE"
      : verdicts.some((item) => item === "POLICY_VIOLATION" || item === "NEEDS_WORK")
      ? "FAIL"
      : verdicts.includes("UNVERIFIABLE")
      ? "UNVERIFIABLE"
      : "PASS";
    verificationRequire(
      canonicalString(aggregate.required_slot_ids) === canonicalString(requiredSlotIds),
      VERIFICATION_CONTEXT_INVALID,
      `$.requirement_aggregates[${index}].required_slot_ids`,
    );
    verificationRequire(
      canonicalString(aggregate.attestation_ids) === canonicalString(matched),
      VERIFICATION_CONTEXT_INVALID,
      `$.requirement_aggregates[${index}].attestation_ids`,
    );
    verificationRequire(
      aggregate.verdict === expectedVerdict,
      VERIFICATION_CONTEXT_INVALID,
      `$.requirement_aggregates[${index}].verdict`,
    );
  });
  ...
  verificationRequire(
    manifestTime !== null && latest !== null && latest <= manifestTime,
