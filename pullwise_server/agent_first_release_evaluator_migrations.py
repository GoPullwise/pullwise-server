...
CURRENT_RELEASE_EVALUATOR_TABLES = (
    ...
    "agent_current_release_gate_sample_sets",
...
_TABLE_STATEMENTS = (
    ...
    """,
    """
    CREATE TABLE IF NOT EXISTS agent_current_release_gate_sample_sets (
        sample_set_digest TEXT PRIMARY KEY
            CHECK(length(sample_set_digest) = 64),
        ...
        policy_ref_size_bytes INTEGER NOT NULL
            CHECK(policy_ref_size_bytes >= 0),
        document_sha256 TEXT NOT NULL CHECK(length(document_sha256) = 64),
        size_bytes INTEGER NOT NULL CHECK(size_bytes >= 0),
        package_identity TEXT NOT NULL,
        package_version TEXT NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS agent_current_release_gate_reports (
        ...
        benchmark_digest TEXT NOT NULL,
        policy_digest TEXT NOT NULL,
        sample_set_digest TEXT NOT NULL,
        benchmark_ref_sha256 TEXT NOT NULL
            CHECK(length(benchmark_ref_sha256) = 64),
        benchmark_ref_size_bytes INTEGER NOT NULL
            CHECK(benchmark_ref_size_bytes >= 0),
        ...
        sample_set_ref_sha256 TEXT NOT NULL
            CHECK(length(sample_set_ref_sha256) = 64),
        sample_set_ref_size_bytes INTEGER NOT NULL
            CHECK(sample_set_ref_size_bytes >= 0),
        ...
        FOREIGN KEY(policy_digest)
            REFERENCES agent_current_release_gate_policies(policy_digest),
        FOREIGN KEY(sample_set_digest)
            REFERENCES agent_current_release_gate_sample_sets(sample_set_digest)
    )
    """,
)
