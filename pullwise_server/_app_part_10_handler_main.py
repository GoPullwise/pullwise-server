...
class PullwiseHandler(BaseHTTPRequestHandler):
        ...
        if path == "/scans":
            ...
            if scan_error is None and scan is None:
                ...
                scan = {
                    ...
                    "quotaBucketIds": quota_result.get("bucketIds") or {},
    ...
    def handle_admin_get(self, segments: list[str], params: dict) -> None:
        ...
        if len(segments) == 3 and segments[:2] == ["admin", "workers"]:
            worker = db.get_worker(segments[2], include_deleted=True, worker_scope=db.WORKER_SCOPE_SHARED)
            if not worker:
                return self.error(HTTPStatus.NOT_FOUND, "Worker not found.")
    ...
    def handle_admin_post(self, segments: list[str], body: dict) -> None:
        ...
        if len(segments) == 4 and segments[:2] == ["admin", "workers"]:
            ...
            if action == "test":
                worker = db.get_worker(worker_id, include_deleted=True, worker_scope=db.WORKER_SCOPE_SHARED)
                if not worker:
    ...
    def handle_admin_worker_command(self, session: dict, worker_id: str, body: dict) -> None:
        ...
        worker = db.get_worker(worker_id, include_deleted=True, worker_scope=db.WORKER_SCOPE_SHARED) or {}
        response = {
            "ok": True,
