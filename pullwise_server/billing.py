...
from .creem_event_rules import (
    PAID_PLAN_IDS, PLAN_IDS, PAID_PLAN_ENTITLEMENT_STATUSES,
    dict_payload, text_payload, object_id, product_payload,
    first_subscription_item, product_payload_from_subscription_item,
    first_text_value, metadata_value, creem_event_subscription_payload,
    creem_event_subscription_id, creem_event_customer_payload, event_created,
    normalize_subscription_status, normalize_creem_subscription_status,
    normalize_plan, normalize_interval,
)
...
def _creem_event_product_bindings() -> dict[str, tuple[str, ...]]:
    return {plan: tuple(creem_configured_paid_product_ids(plan)) for plan in PAID_PLAN_IDS}


def creem_product_configured_for_plan(product: dict | None, plan: str) -> bool:
    return creem_rules.creem_product_configured_for_plan(product, plan, _creem_event_product_bindings())
...
def creem_plan_from_product(product: dict | None) -> str | None:
    return creem_rules.creem_plan_from_product(product, _creem_event_product_bindings())


def billing_update_from_creem_event(event: dict) -> dict | None:
    return creem_rules.billing_update_from_creem_event(event, _creem_event_product_bindings())


def interval_from_configured_creem_product_id(product_id: object) -> str | None:
    return creem_rules.interval_from_configured_creem_product_id(product_id, _creem_event_product_bindings())


def interval_from_creem_product(product: dict | None) -> str | None:
    return creem_rules.interval_from_creem_product(product, _creem_event_product_bindings())
