...
WORKER_INSTANCE_CONFIG_BASE_DIR = "/etc/pullwise-worker"
WORKER_INSTANCE_DATA_BASE_DIR = "/var/lib/pullwise-worker"
WORKER_INSTANCE_LOG_BASE_DIR = "/var/log/pullwise-worker"
WORKER_INSTANCE_SYSTEMD_DIR = "/etc/systemd/system"
WORKER_INSTANCE_LOGROTATE_DIR = "/etc/logrotate.d"
WORKER_INSTANCE_BIN_DIR = "/usr/local/bin"
WORKER_INSTANCE_SERVICE_PREFIX = "pullwise-worker-"
WORKER_INSTANCE_SERVICE_GROUP = "pullwise-worker"


def worker_instance_service_name(worker_id: object) -> str:
    safe_worker_id = worker_safe_service_id(worker_id)
    if not safe_worker_id:
        raise ValueError("worker id does not contain any safe service-name characters")
    return f"{WORKER_INSTANCE_SERVICE_PREFIX}{safe_worker_id}"


def worker_instance_service_user(worker_id: object) -> str:
    safe_worker_id = worker_safe_service_id(worker_id)
    suffix = "".join(char for char in safe_worker_id.replace("_", "-").lower() if char.isalnum() or char == "-")[:16]
    return f"pw-worker-{suffix or 'worker'}"


def worker_instance_paths(worker_id: object) -> dict[str, str]:
    safe_worker_id = worker_safe_service_id(worker_id)
    if not safe_worker_id:
        raise ValueError("worker id does not contain any safe service-name characters")
    service_name = worker_instance_service_name(worker_id)
    return {
        "configDir": os.path.join(WORKER_INSTANCE_CONFIG_BASE_DIR, safe_worker_id),
        "dataDir": os.path.join(WORKER_INSTANCE_DATA_BASE_DIR, safe_worker_id),
        "logDir": os.path.join(WORKER_INSTANCE_LOG_BASE_DIR, safe_worker_id),
        "serviceFile": os.path.join(WORKER_INSTANCE_SYSTEMD_DIR, f"{service_name}.service"),
        "logrotateFile": os.path.join(WORKER_INSTANCE_LOGROTATE_DIR, service_name),
        "binPath": os.path.join(WORKER_INSTANCE_BIN_DIR, service_name),
    }


def worker_instance_path_inside(path: str, root: str) -> bool:
    ...
    try:
        ...
        return False
    return os.path.normcase(common) == os.path.normcase(root_abs) and os.path.normcase(path_abs) != os.path.normcase(root_abs)


def remove_worker_instance_dir(path: str, root: str, removed: list[str]) -> None:
    if not worker_instance_path_inside(path, root):
        raise ValueError(f"refusing to remove unexpected worker directory: {path}")
    if os.path.islink(path):
        raise ValueError(f"refusing to remove symlinked worker directory: {path}")
...
def remove_worker_instance_file(path: str, root: str, removed: list[str]) -> None:
    if not worker_instance_path_inside(path, root):
        raise ValueError(f"refusing to remove unexpected worker file: {path}")
...
def worker_service_user_is_shared(worker_id: object) -> bool:
    service_user = worker_instance_service_user(worker_id)
    current_worker_id = public_issue_text(worker_id)
    for worker in db.list_workers():
        other_worker_id = public_issue_text(worker.get("worker_id"))
        if other_worker_id and other_worker_id != current_worker_id and worker_instance_service_user(other_worker_id) == service_user:
            return True
    return False


def cleanup_admin_worker_instance(worker_id: object) -> dict:
    service_name = worker_instance_service_name(worker_id)
    service_user = worker_instance_service_user(worker_id)
    ...
    remove_worker_instance_file(paths["binPath"], WORKER_INSTANCE_BIN_DIR, removed)
    remove_worker_instance_dir(paths["configDir"], WORKER_INSTANCE_CONFIG_BASE_DIR, removed)
    ...
    return {
        ...
        "serviceUserRemoved": service_user_removed,
